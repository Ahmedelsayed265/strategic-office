import type { PieChartResponse } from "@/types/types";
import { postRequest } from "@/lib/axiosApi";
import { useQuery } from "@tanstack/react-query";
import { useSearchParams } from "react-router";

export default function useGetPieChartData() {
  ...
  const { isLoading, data, isError } = useQuery({
    queryKey: ["pie-chart-data", payload],
    queryFn: (): Promise<PieChartResponse> =>
      postRequest<PieChartResponse>(
        "/IndicatorsApi/get-filtered-pie-data",
        payload
      ),
    enabled: !!indicatorId,
  });
